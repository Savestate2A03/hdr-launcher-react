  ...
  useEffect(() => {
    Github.pullRequests()
      .then((json) => {
        ...
        const filtered = (json as any[]).filter((pr) => !pr.draft);
    ...
    <div className="overlay-progress scroll-hidden">
      ...
      <div className="border-bottom">
        ...
        <p id="title" className="header-item">
          Pull Request Installation Menu
        </p>
      </div>
      {pullRequests !== null ? (
        <div className="stage-config-body border-bottom">
          <div className="scrolling-fit-nobar left-side">
            {pullRequests.map((pr) => (
              <ScrollFocusButton
                className={`longer-main-button${
                  Backend.isSwitch() ? ' no-transition' : ''
                }`}
                onFocus={() => {
                  setHoveredPull(pr);
                }}
                text={`${
                  pr.title.length < 27
                    ? pr.title
                    : `${pr.title.substring(0, 24)}...`
                } | #${pr.number}\u00A0`}
                onClick={async () => {
                  try {
                    setProgress(
                      new Progress('Installing PR', 'Installing a PR', 0),
                    );
                    ...
                    comments.forEach(async (comment) => {
                      ...
                      const endUrl = body.indexOf(')', startUrl);
                      const url = body.substring(startUrl, endUrl);
                      if (url.length === 0) {
                        alert(
                      ...
                      setProgress(
                        new Progress('Extracting PR', 'Extracting', 0),
                      );
                      const exists = await backend.fileExists(
                        `${root}ultimate/mods/hdr-pr`,
                      );
                      if (exists) {
                        await backend.removeDirAll(
                          `${root}ultimate/mods/hdr-pr`,
                        );
                      }
                      ...
                      if (
                        pr.labels.filter((label: any) =>
                          label.name.includes('includes assets'),
                        ).length &&
                        pr.body.includes('.zip](https://')
                      ) {
                        console.info('beginning handling of hdr-assets-pr');
                        setProgress(
                          new Progress(
                            'Installing PR Assets',
                            'Getting PR assets',
                            0,
                          ),
                        );
                        ...
                        const startParse = pr.body.indexOf('.zip](https://');
                        const startUrl = pr.body.indexOf(
                          'https://',
                          startParse,
                        );
                        const endUrl = pr.body.indexOf(')', startUrl);
                        const url = pr.body.substring(startUrl, endUrl);
                        if (url.length === 0) {
                          alert(
                        ...
                        let exists = await backend.dirExists(
                        ...
                        console.info(
                          'checking if a pr-assets.zip already exists',
                        );
                        exists = await backend.dirExists(
                        ...
                        alert(
                          "Pull request and assets downloaded! Please disable 'hdr' and 'HDR Assets' in Arcadia, and enable 'hdr-pr' and 'hdr-assets-pr' instead.",
                        );
                      ...
                      alert(
                        "Pull request downloaded! Please disable 'hdr' in Arcadia, and enable 'hdr-pr' instead.",
                      );
                      backend.openModManager();
                      setProgress(null);
                    });
        ...
        <h1 style={{ color: 'white' }}>Getting pull requests...</h1>
      )}
      <LogPopout />
    </div>
  );
