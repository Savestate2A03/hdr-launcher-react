...
const dev_nro_path =
  'atmosphere/contents/01006A800016E000/romfs/smashline/development.nro';
...
export default async function verify(
  ...
  try {
    ...
    for (let i = 0; i < hdr_folders.length; i++) {
      const folder = hdr_folders[i];
      reportProgress(
        new Progress(
          `Checking ${folder}`,
          `Folder: ${folder}`,
      ...
      for (let i = 0; i < hdr_files.length; i++) {
        const element = hdr_files[i];
        if (
          !expected_files.includes(element) &&
          always_ignore_files.filter((item) => element.includes(item)).length ==
            0
        ) {
    ...
    if (
      Backend.isNode() &&
      (await backend.dirExists(sdroot + deprecated_ryu_mods_dir))
    ) {
      await backend
    ...
    if (Backend.isSwitch()) {
      ...
      if (Number(api_version[0]) >= 1 && Number(api_version[1]) >= 7) {
        ...
        const hdr_enabled = await backend.isModEnabled('sd:/ultimate/mods/hdr');
    ...
    if (result_str.length == 0) {
      return 'Your installation should be clean and correct.';
    }
    return result_str;
  } catch (e) {
    return `error during verify: ${e}`;
  }
}
